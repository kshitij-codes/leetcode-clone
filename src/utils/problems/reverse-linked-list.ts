...
export const reverseLinkedListHandler = (fn: any) => {
  try {
    ...
    for (let i = 0; i < tests.length; i++) {
      // we convert array to linked list
      const list = createLinkedList(tests[i]);
      // and pass it to user's written code
      const result = fn(list);
      // we convert the linked list back to array and check whether result matches with the correct answer
